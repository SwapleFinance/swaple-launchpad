...
const handleData = async () => {
  try {
   ...
   if(account){
    const pend = await Stakes.methods.pendingReward(pid,account).call();
    ...
    const date1 = ((parseInt(pool.lockedPeriod)*1000) -  Date.now()) /1000/60/60/24;
    const datee = new Date(parseInt(pool.lockedPeriod)*1000);
    console.log("date of new : ", )
    setDays((date1.toFixed()))
    if(date1 <= 0){
        setStatus(true)
    }
    const unstakeDate = `${datee.getDate()}/${datee.getMonth()}/${datee.getFullYear()}`;
      ...
      <Flex justifyContent='space-between' alignItems='center' className="mb_end mt-top-end">
       ...
       <div className="card-det">
              ...
              <div className="card-d">
                ...
                <Text bold style={{ display: 'flex', alignItems: 'center', wordBreak: 'break-all', color:'#fff' }}>
